...
- **Admin role assignment restriction**: Only users with `rol_id === 3` (admin) can assign the admin role. `UsuariosTable` receives `usuarioRolId` as a prop from the server component and filters the role `<select>` to exclude `rol.id === 3` for non-admins. Additionally, `updateUsuario` in `usuarios.client.ts` validates server-side that the current user is admin before allowing a `rol_id = "3"` update. Moderators can only assign `cliente` and `moderador` roles.
