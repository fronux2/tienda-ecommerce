...
interface PedidoRecibidoAdminProps {
  pedidoId: string
  clienteEmail: string
  items: ItemAdmin[]
  total: number
...
export default function PedidoRecibidoAdmin({
  pedidoId,
  clienteEmail,
  items,
  total,
  direccion,
  fecha,
}: PedidoRecibidoAdminProps) {
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.headerTitle}>¡Nuevo Pedido!</h1>
        <p style={styles.headerSub}>Se ha recibido un nuevo pedido en Tienda Mangas</p>
      </div>

      <div style={styles.body}>
        <div style={styles.alertBox}>
          El pedido <strong>#{pedidoId}</strong> está pendiente y necesita ser procesado.
        </div>

        <p style={styles.label}>Cliente</p>
        <p style={styles.value}>{clienteEmail}</p>
